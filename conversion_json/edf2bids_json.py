...
print("Next: Load the manual_metadata.yml file with the additional metadata. \
      This file must contain at least the additional REQUIRED metadata in the format specified in the BIDS specification. \
      Please enter the required metadata manually before loading the file in a next step.")
...
ScreenResolution = list(map(int,(df_ms_reduced[df_ms_reduced[2] == "DISPLAY_COORDS"].iloc[0:1, 3:5].to_string(header=False, index=False)).split(" ")))

TaskName =' '.join([ts for ts in events if ts.startswith("** RECORDED BY")]).replace("** RECORDED BY ","").replace("\n","")

#Eyetrack.json Metadata

ManufacturersModelName =' '.join([ml for ml in events if ml.startswith("** EYELINK")]).replace("** ","").replace("\n","")

DeviceSerialNumber =' '.join([sl for sl in events if sl.startswith("** SERIAL NUMBER:")]).replace("** SERIAL NUMBER: ","").replace("\n","")

SamplingFrequency = int(df_ms_reduced[df_ms_reduced[2] == "RECCFG"].iloc[0:1, 2:3].to_string(header=False, index=False))

eye = df_ms_reduced[df_ms_reduced[2] == "RECCFG"].iloc[0:1, 5:6].to_string(header=False, index=False)
if eye == "L": 
...
if df_ms[df_ms[3]=="VALIDATION"].empty == False:
    AverageCalibrationError = np.array(df_ms[df_ms[3]=="VALIDATION"][[9]]).astype(float).tolist()
else:
    AverageCalibrationError = []    

CalibrationCount = len(df_ms_reduced[df_ms_reduced[3] == "CALIBRATION"])

cal_pos= np.array(df_ms_reduced[df_ms_reduced[2] == "VALIDATE"][8].str.split(",",expand=True)).astype(int).tolist()
...
CalibrationType = df_ms_reduced[df_ms_reduced[3] == "CALIBRATION"].iloc[0:1, 2:3].to_string(header=False, index=False)

if len(cal_pos) != 0:
    cal_unit=(df_ms_reduced[df_ms_reduced[2] == "VALIDATE"][[13]]).iloc[0:1,0:1].to_string(header=False, index=False)
    if cal_unit == "pix.":
...
EyeTrackingMethod = pd.DataFrame(' '.join([tm for tm in events if tm.startswith(">>>>>>>")]).replace(")",",").split(",")).iloc[1:2].to_string(header=False, index=False)

if df_ms[df_ms[3]=="VALIDATION"].empty == False:
    MaximalCalibrationError = np.array(df_ms[df_ms[3]=="VALIDATION"][[11]]).astype(float).tolist()
else:
    MaximalCalibrationError = []


PupilFitMethod = (df_ms_reduced[df_ms_reduced[2] == "ELCL_PROC"]).iloc[0:1, 1:2].to_string(header=False, index=False)

StartTime=np.array(pd.DataFrame([st.split() for st in events if st.startswith("START")])[1]).astype(int).tolist() #TODO:figure out if this is actually the StartTime meant by the specification

StopTime=np.array(pd.DataFrame([so.split() for so in events if so.startswith("END")])[1]).astype(int).tolist() #TODO:figure out if this is actually the StopTime meant by the specification 
